...
class HQForwardTimer extends React.Component {
	constructor() {
		super( ...arguments );
		this.mounted = false;
		this.state = {
			...
			passedSecond: this.props.initialSecondsRemaining
		};
		this.tick = () => {
			...
			const passedSecond = Math.max( this.state.passedSecond + dt, 0 );
			if ( this.mounted ) {
			...
			if ( this.props.onTick ) {
				this.props.onTick( passedSecond );
			}
		};
	...
	render() {
		...
		const passedSecond = this.state.passedSecond;
		const style = this.props.style;
		return ( <react_native_1.Text allowFontScaling={allowFontScaling} style={style}>
			{this.getFormattedTime( passedSecond )}
	</react_native_1.Text> );
	}
}

HQForwardTimer.defaultProps = {
	interval: 1000,
	formatSecondsRemaining: null,
	onTick: null,
	onTickWithCurrentTime: null,
	onTimeElapsed: null
};
exports.default = HQForwardTimer;
